...
/**
 * Lightweight 1D LED-strip effect simulator for UI preview
 * (similar spirit to WLED Liveview pixel strip + simplified FX families).
 *
 * Not a 1:1 port of FX.cpp — that would be tens of thousands of lines.
 * Each effectId maps to a recognizable pattern family.
 */
object EffectPreviewEngine {

    data class Params(
        val effectId: Int,
        val paletteId: Int = 6, // Party looks good for demos
        val speed: Float = 128f,      // 0..255
        val intensity: Float = 128f,  // 0..255
    ...
    /**
     * @param timeSec continuous time in seconds
     * @return colors for led indices 0 until ledCount
     */
    fun render(params: Params, timeSec: Float): List<Color> {
        ...
        val id = params.effectId.coerceIn(0, 159)
        ...
        fun scale(c: Color, s: Float) = Color(c.red * s, c.green * s, c.blue * s, 1f)

        return List(n) { i ->
            val x = i / (n - 1).coerceAtLeast(1).toFloat()
            pixel(id, i, n, x, t, sp, ix, params.primary, params.secondary, ::pal, ::mix, ::scale)
        }
    }
